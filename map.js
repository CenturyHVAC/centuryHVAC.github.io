...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeFirebaseAndMap() {
    // Initialize Firebase
    ...
    // Initialize map with rotation plugin and increased max zoom
    ...
    rotationSlider.addEventListener('input', (e) => {
      ...
      enhancedSaveMapProgress(); // Save state after rotation change
    });

    // Add event listeners for map state changes
    map.on('moveend', enhancedSaveMapProgress);
    map.on('zoomend', enhancedSaveMapProgress);
    map.on('rotate', enhancedSaveMapProgress);
    ...
    saveStatusBtn.addEventListener('click', () => {
      if (currentMarker && currentMarker.markerData) {
        ...
        enhancedSaveMapProgress(); // Save state after updating status
      }
    });
    ...
    saveNewPartBtn.addEventListener('click', () => {
      ...
      if (partName && partNumber && currentMarker && currentMarker.markerData) {
        ...
        enhancedSaveMapProgress(); // Save state after updating parts
      }
    });
...
Freon Type:   ${currentMarker.markerData.details.freonType || 'N/A'}`.trim();
        ...
        enhancedSaveMapProgress(); // Save state after updating details
      }
    });
        ...
        currentMarker.markerData.parts.forEach((part, index) => {
          ...
          deleteButton.addEventListener('click', () => {
            ...
            if (partIndex !== undefined && partIndex >= 0 && partIndex < currentMarker.markerData.parts.length) {
              ...
              populatePartsList(partsList, currentMarker, editingPartIndex, partNameInput, partNumberInput, partReplacementDateInput, addPartsInput, showAddPartsBtn, saveNewPartBtn);
              updatePartsHistory(partsHistoryContainer, currentMarker);
              enhancedSaveMapProgress(); // Save state after deleting part
            }
          });
...
Freon Type:   ${currentMarker.markerData.details.freonType || 'N/A'}`.trim();
    ...
    mapRef.once('value').then((snapshot) => {
      const savedMapState = snapshot.val();
      if (savedMapState) {
        // Restore map view
        map.setView(savedMapState.center, savedMapState.zoom);
        map.setBearing(savedMapState.bearing);
        ...
        savedMapState.markers.forEach(markerData => {
          ...
          .on('click', function(markerEvent) {
            // Existing marker click handler logic
            markerEvent.originalEvent.stopPropagation();
            if (!this.markerData) {
    ...
    mapRef.on('value', (snapshot) => {
      const savedMapState = snapshot.val();
      if (savedMapState) {
        // Update markers from Firebase (if needed)
        // This can be expanded based on your real-time update requirements
      }
    });
  }
});